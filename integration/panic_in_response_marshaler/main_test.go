...
import (
	...
	strings_pb "github.com/utrack/clay/integration/panic_in_response_marshaler/pb"
	strings_srv "github.com/utrack/clay/integration/panic_in_response_marshaler/strings"
)
...
func testServer(t *testing.T) *httptest.Server {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		defer func() {
			...
			if expected := `{"stdtime":`; expected != string(got) {
				t.Errorf("expected response: `%s`, got: `%s`", expected, got)
			}
			p := recover()
			expected := "interface conversion: *time.Time is not proto.Message: missing method ProtoMessage"
			if err, ok := p.(*runtime.TypeAssertionError); ok && err.Error() == expected {
				return
			}
			t.Fatalf("expected panic: `%v`, got: `%v`", expected, p)
		}()
